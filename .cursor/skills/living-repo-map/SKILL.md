...
This is not a wiki and not a local CLI dump. The map is files in the tree. Cloud Agents clone the repo, so they can read it on Monday.

## Hard rules

- Do not clone the repo onto the operator machine as the authoring workspace. Cloud Agents against the connected GitHub (or Origin) repo are the checkout.
...
- Do not create GitHub PATs. Do not create Slack bots.
- Propose before a Cloud Agent launch if the user has not already asked for that launch.
- Human reviews the PR. Nothing merges until they say so.
...
3. Optional KTLO targets the user already cares about (deprecated APIs, a CVE-shaped area, a migration). If none, the map still produces blast-radius rows; do not invent incidents.
...
2. Write a static site in `demo-map/`:
   - `index.html` hub
   ...
   - `graph.json` (and vendor any JS into `demo-map/` so `file://` works)
   - `styles.css`
3. Write `MAP.md` at repo root. Machine-readable. Sections that match the pages. Each entry: `id`, `title`, `paths`, `touches`, `ktlo_notes`, and a `kickoff` fenced prompt the next Cloud Agent can paste.
4. Vanilla HTML plus CSS plus SVG. No app build. Opening `demo-map/index.html` in a browser must work.
...
7. Touch no product code except adding `demo-map/` and `MAP.md`.

Keep the prompt tight. Repeat the in-scope and out-of-scope lists. Say: never invent files.

## Step 3. Verify before anyone films or kicks work
...
If more than two nodes are wrong, send a punch list to the same Cloud Agent. Do not kick Act 2 on a fictional map.

Host if you need it clickable: GitHub Pages from the map branch `/demo-map`, or a zip of `demo-map/` opened as local `index.html`. Do not claim Cursor hosted the site.

## Step 4. The map is the interface

Do not build a second product UI.

- Developers open `demo-map/index.html` (hosted or local).
...
1. Read `MAP.md` first, then `demo-map/blast-radius.html` and `demo-map/graph.json`. Treat those as the architecture. Do not rediscover the repo.
...
Do not merge. Run Bugbot on the PR (`bugbot run` or `cursor review` if nothing posts). A human reviews in the IDE with Agent on one hunk and Tab on one edit.
...
## On a tape or live demo

Kick the skill (or the map Cloud Agent) live. Do not stare at thinking logs. Cut to the already-landed map from the same prompt if the live run is slow. Kick one work Cloud Agent live from a hotspot. Backup PR is mandatory on a large repo.

## Do not say

- That a Cloud Agent PR should merge without a human
- That Cursor hosted the map
- Model names, context-window sizes, or pricing
- "Background Agents"
- Exact line counts beyond a round number the user already approved
